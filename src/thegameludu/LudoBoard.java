...
//                //up row3
    ...
    public boolean movePiece(int pieceNumber,int number,String color)
    {
            ...
            if(!this.getPiece(0, color).activated)
                {
                    ...
                    if(fields[p.boardpoint].listOfPieces.size()>0)
                        fields[p.startAt].listOfPieces.add(fields[p.boardpoint].listOfPieces.remove(0));
                    else
                    {
                        fields[p.startAt].listOfPieces.add(p);
                    }
        ...
        if(p.score<=50)
        {
            ...
            else
            {
                ...
                if(fields[old].listOfPieces.size()>0)
                    fields[p.boardpoint].listOfPieces.add(fields[old].listOfPieces.remove(0));
                else
                {
                    fields[p.boardpoint].listOfPieces.add(p);
                }
    ...
    {
        System.out.println("I am outside 1");
        if(fields[p.boardpoint].listOfPieces.size()>0)
        {
            System.out.println("I am outside 2");
            for(int i=0;i<fields[p.boardpoint].listOfPieces.size();i++)
            {
                System.out.println("I am outside 3 : loop#"+i);
                System.out.println("host color #"+p.color);
                System.out.println("host boardpoint #"+p.boardpoint);
                
                
                System.out.println("target color #"+fields[p.boardpoint].listOfPieces.get(i).color);
                System.out.println("target boardpoint #"+fields[p.boardpoint].listOfPieces.get(i).boardpoint);
                
                System.out.println("stop ?"+fields[p.boardpoint].isStop);
                
                if( !p.color.equals(fields[p.boardpoint].listOfPieces.get(i).color) && !fields[p.boardpoint].isStop )
                {
                    System.out.println("I am inside");
                    fields[p.boardpoint].listOfPieces.get(i).x=fields[p.boardpoint].listOfPieces.get(i).HomeX[i];
                    fields[p.boardpoint].listOfPieces.get(i).y=fields[p.boardpoint].listOfPieces.get(i).HomeY[i];
                    fields[p.boardpoint].listOfPieces.get(i).activated=false;
                    
                    
                    fields[fields[p.boardpoint].listOfPieces.get(i).initialHomePoint].listOfPieces.add(fields[p.boardpoint].listOfPieces.remove(i));
    ...
    public void mouseClicked(MouseEvent e) 
    {
        if(diceRolled&&turn==0)
        {
            ...
            for(int i=0;i<4;i++)
            {
                ...
                System.out.println("Green.x : "+ fields[green_pieces[i].boardpoint].x+", Greeen.y: "+fields[green_pieces[i].boardpoint].y );
                
                if((green_pieces[i].x<e.getX() && e.getX()<green_pieces[i].x+30) && (green_pieces[i].y<e.getY() && e.getY()<green_pieces[i].y+60)) 
                {
                    boolean doneMove=true;
                    while(!movePiece( green_pieces[i].pieceNumber ,DiceResultforGreen,"GREEN"))
                    {
                        JOptionPane.showInputDialog("Please select any other piece, this move is not possible ");
